...
本仓库是 Dreambo 双足机器人步态策略的 **运行时 SDK**（Runtime SDK），负责把 `rsl_rl` 在 Isaac Lab 中训练得到的 ONNX 步态网络部署到真实硬件：在树莓派（或同等 Linux 单板机）上以 50 Hz 跑策略推理、200 Hz 下发 CAN 指令到电机，并通过手柄完成开关机/急停/姿态切换。
...
| IMU         | N100（CP2102 USB-UART，921600 bps）                         |
...
### 怎么确定真机 IMU 的实际朝向（这才是关键）

URDF 的 `rpy="0 0 0"` 只是设计意图——"我们假设你把 IMU 这样装"。但物理上，工厂、PCB layout、外壳、装配方向都可能让真机芯片朝向跟 URDF 不一致。所以**必须实测**。下面是四步走：

#### 步骤 A：静止读 `proj_g`，确认 Z 轴

机器人水平静放在地面上，跑 `imu_dump`。看 `read()` 返回的 `proj_g`：

| 现实情况                    | 期望读数                              |
|-----------------------------|---------------------------------------|
| chip Z 朝上（=世界上方）    | `proj_g ≈ (0, 0, -1)`                 |
| chip Z 朝下（=世界下方）    | `proj_g ≈ (0, 0, +1)`                 |
| chip 倒着装绕 X 翻 180°     | `proj_g ≈ (0, 0, +1)`，且 Y 也会翻    |
| chip 立着装、Z 水平         | `proj_g ≈ (±1, 0, 0)` 或 `(0, ±1, 0)` |

参考一次实测 dump：

```text
proj_g=[x=+0.226, y=+0.079, z=-0.971]
```

`z ≈ -0.971` 表示 chip Z 几乎跟世界 Z 同向——机器人没有完全水平（机身略前倾），但 chip Z 是 base 系 +Z。**这一项对了**。

#### 步骤 B：前倾测试，确认 X 轴

把机器人向前倾 30° 左右（机器人沿世界 +X 方向倒下）：

| 期望读数（chip X 跟 base X 同向） | 实际                                              |
|-----------------------------------|---------------------------------------------------|
| `proj_g_x` 变成 **正值**，约 +0.5 | ✓ 对了，base 系 +X = 机身正前方                   |
| `proj_g_x` 变成 **负值**          | ✗ chip X 反了，需要在 `R_base_from_imu` 里 X 取反 |

#### 步骤 C：左倾测试，确认 Y 轴

把机器人向左倾 30°（机器人沿世界 +Y 方向倒下）：

| 期望读数（chip Y 跟 base Y 同向，base +Y = 左） | 实际             |
|-------------------------------------------------|------------------|
| `proj_g_y` 变成 **正值**                        | ✓ 对了           |
| `proj_g_y` 变成 **负值**                        | ✗ chip Y 反了    |

#### 步骤 D：摇摆测试，确认 `ang_vel` 符号

机器人静止，用手绕机身 +Z 轴正方向（俯视图逆时针）转动：

| 期望（同向）                                   | 实际                                                          |
|------------------------------------------------|---------------------------------------------------------------|
| `ang_vel_z > 0`（右手定则，+Z 旋转 = 正值）    | ✓                                                             |
| `ang_vel_z < 0`                                | ✗ chip Z 旋转方向反了，**仅 `ang_vel` 取反不够，得整体重新算 R** |

> 任何一步出现 ✗，都需要修改 `imu.py:R_base_from_imu`。注意：默认实现已经把 `R_base_from_imu` 同时作用于 `proj_g` 与 `ang_vel`，所以**只改观测里的某一个分量是治标不治本**——一定要把矩阵本身改对。修改后重跑步骤 A→D 直到全部 ✓ 再上电跑策略。
