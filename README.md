...
本仓库是 OLAF 双足机器人步态策略的 **运行时 SDK**（Runtime SDK），负责把 `rsl_rl` 在 Isaac Lab 中训练得到的 ONNX 步态网络部署到真实硬件：在树莓派（或同等 Linux 单板机）上以 50 Hz 跑策略推理、200 Hz 下发 CAN 指令到电机，并通过手柄完成开关机/急停/姿态切换。
